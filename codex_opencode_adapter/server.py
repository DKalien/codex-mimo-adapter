...
class Adapter:
    ...
    def stream(self, body: JSON, emit) -> JSON:
        ...
        try:
            ...
            LOG.error(
                "upstream_stream_failed status=%s response_id=%s error=%s",
                error.status,
                assembler.response_id,
                error.message,
            )
            return assembler.fail("upstream_error", error.message)
        except Exception as error:
            LOG.exception(
                "stream_finalize_failed response_id=%s", assembler.response_id
            )
            return assembler.fail("internal_error", str(error))
