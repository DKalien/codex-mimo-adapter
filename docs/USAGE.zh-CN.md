# Codex MiMo Token Plan Adapter 使用说明
...
普通 Windows 用户不需要 Rust；请直接使用 [Windows 图形启动器部署](PORTABLE.zh-CN.md) 的六步流程。

如果你需要用 CLI，先看这 3 步就够了：
...
2. 初始化全局配置（安装用户级 Provider + 写入 9 个全局受管理 agent）：

```powershell
$env:MIMO_API_KEY = "<你的 MiMo Token Plan API Key>"
...
```powershell
./scripts/dev-run.ps1 -ApiKey "<你的 MiMo Token Plan API Key>"
```
...
- `codex-mimo-adapter global-init`
- `codex-mimo-adapter run` 或 `codex-mimo-adapter start`

`check` 主要用于排障，`auth print-local-token` 主要给 provider auth helper 调用，不是日常手工入口。

> `auth print-local-token` 输出 adapter 本地 token，用于证明调用方可以访问本机 adapter；默认全局路由为 `model = "mimo_adapter/global/mimo/<model-id>"`，不依赖项目注册表。裸 `mimo/<model-id>` 已不再支持。项目级路由仅供高级用法兼容保留。
...
| `global.env` | `%USERPROFILE%\.codex-mimo-adapter\global.env` | 保存全局本地 token 和运行参数；不保存 API Key |
| Agent TOML | `%USERPROFILE%\.codex\agents\*.toml` | 定义 9 个受管理子代理，并把 `model` 写成稳定全局路由 |
| `.codex-mimo-adapter.env` | 当前项目根目录 | 仅供高级项目级路由保存项目 API Key、项目 ID 与本地 token |
...
- 创建或覆盖 `%USERPROFILE%\.codex-mimo-adapter\global.env`，其中不含 API Key
- 创建或覆盖以下 9 个受管理 agent 到 `%USERPROFILE%\.codex\agents\`：`default`、`explorer`、`worker`、3 个 `oss_worker_pro_*` 与 3 个 `oss_worker_std_*`；不会覆盖其他用户自建的 agent 文件
- 把 agent `model` 写成 `mimo_adapter/global/mimo/<model-id>`

其中本地 token 会生成并落盘到 `global.env`；它不是 MiMo API Key。MiMo API Key 必须保留在启动适配器的进程环境中；图形启动器会从 DPAPI 安全保存中注入它。
...
cargo run -- init --api-key "<你的 MiMo Token Plan API Key>"
cargo run -- start
```
...
- 默认配置保存在当前项目的 `.codex-mimo-adapter.env`。
- 默认端口为 `4010`。
- Bridge 必须保持运行，Codex 子代理才能连接。
- `CODEX_MIMO_MAX_CONCURRENCY` 是最大并发数，默认 `8`，可手动改 `.codex-mimo-adapter.env`。
...
```powershell
Invoke-RestMethod http://127.0.0.1:4010/v1/models -Headers @{Authorization = "Bearer $(codex-mimo-adapter auth print-local-token)"}
```
...
请求侧 provider-specific reasoning 参数仍需真实 MiMo Token Plan 验证。不要通过增加复杂提示词来猜测 reasoning 是否生效。
...
Codex provider auth 的 token 必须与项目 `.codex-mimo-adapter.env` 里的
`CODEX_MIMO_LOCAL_TOKEN` 相同。不要把 MiMo Token Plan API Key 配成本地 token。
...
检查启动 Bridge 的进程是否设置了 `MIMO_API_KEY`。不要在日志或聊天中粘贴
API Key。
...
如果 MiMo Token Plan 的模型列表有更新，不要先改代码，先看：
...
1. 如果只是新增模型：
   直接把 `.codex/agents/*.toml` 里的 `model` 改成新的 `mimo_adapter/<project_key>/mimo/<model-id>`；更稳妥的做法是重新执行 `init` 生成模板。
2. 如果旧模型下线或改名：
...
```powershell
./scripts/run-real-smoke.ps1 -ApiKey "<你的 MiMo Token Plan API Key>"
```
...
完成本地 mock 测试后，再进入真实 MiMo Token Plan / Codex subagent 验证。

完整步骤见 [VALIDATION.zh-CN.md](VALIDATION.zh-CN.md)。

真实验证应按顺序执行：

1. 启动 adapter，并设置 `RUST_LOG=codex_mimo_adapter=debug`。
2. 检查 `/health`。
3. 检查 `/v1/models`。
